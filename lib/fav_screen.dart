...
class _FavScreenState extends State<FavScreen> {
  ...
  void initState() {
    ...
    loadNSFWPreference();
    loadFavorites('loading').then((value) => loadNSFWPreference());
    subscribe();
    _scrollController.addListener(() {
      if (_scrollController.position.pixels ==
          _scrollController.position.maxScrollExtent) {
        // The user has scrolled to the end, load more items
        loadMoreFavorites();
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final filteredFavorites = isNSFWEnabled
        ? favorites
        : favorites
            .where((favorite) =>
                !favorite.imageUrl.contains("slxftlarogkbsdtwepdn.supabase.co"))
            .toList();
    return Scaffold(
      body: Stack(
        children: [
